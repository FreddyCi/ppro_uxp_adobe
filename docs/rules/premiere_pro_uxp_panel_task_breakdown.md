...
**Estimate:** 120 minutes
**Progress Note:** [Progress note for this task]
**Description:** Automate importing locally generated MP4 assets into Premiere, place them on a target sequence, and inject timeline markers and clip metadata so editors can review AI outputs without manual prep work.
**Deliverables:**
- Local media discovery pipeline that surfaces MP4s generated by the Bolt hybrid storage flow (configurable watch folder + file picker UI).
- Premiere import workflow that moves the selected MP4 into the project bin using `PremiereService.importAsset`, respecting bin naming conventions and avoiding duplicate imports.
- Timeline automation that either appends the clip to the active sequence or spins up a new sequence (matching clip settings) when one is not available.
- Marker authoring that applies at least: start/end clip markers, generation metadata marker (e.g., prompt, seed, duration), and optional beat markers read from sidecar JSON if present.
- Metadata embedding that tags the clip with generation details (prompt, model, timestamp) using XMP or project item comment fields for later filtering.
- Progress + error feedback surfaced in the UI (toast + status bar) and logged for troubleshooting.
**Acceptance Criteria:**
- Selecting a locally-stored MP4 through the panel results in a new project item without duplicate imports when the same file is chosen twice.
- The imported clip lands on the correct sequence track at the next available edit time (or in a new sequence when none exists) with no offset or truncation.
- Timeline markers appear at 00:00 and clip end, include labeled metadata, and any optional beat markers align with the provided timestamps.
- Clip/project metadata displays the prompt/model info inside Premiere’s metadata panel after import.
- Users receive clear progress updates and actionable errors if media is missing, locked, or incompatible.
- Workflow remains functional in offline mode (no Azure calls) and respects the storage mode toggle introduced in T023.5.
**Coding Rules:**
- Reuse Bolt hybrid helpers for filesystem access; avoid new native bridges unless required.
- Keep Premiere API interactions within `PremiereService`; UI layer should consume the service.
- Do not over engineer
---
